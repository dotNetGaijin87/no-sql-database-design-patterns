...
because `items[]` is array-valued).

`model-embedding.svg` is a third, complementary diagram (an "aggregate" view) that
foregrounds embedding: each collection is a green document box and embedded
sub-documents / arrays are nested boxes. It is generated by
[`embedding.gen.js`](embedding.gen.js) (`node docs/images/embedding.gen.js`).

These are the editable sources that sit next to the rendered SVGs, mirroring the way the
